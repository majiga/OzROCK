...
print(classification_report(labels_expert, labels_merged, digits=2)) #, average='micro')) #, digits=2))
...
auto_dictionary = r"Files/.txt"
auto_model = "Files/.txt"
auto_merged = r"Files/auto_labelled_merged.txt"

merge_labels(auto_dictionary, auto_model, auto_merged)
"""
...
filename = r"C:\wamex\data\merged_labels.txt"

df = pd.read_csv(filename, quoting=csv.QUOTE_NONE, header=None, encoding='Latin1', skip_blank_lines=True, sep=' ')
df = df.replace(np.nan, '', regex=True)
#df = pd.read_csv(filename, header=None, sep=' ', encoding='latin1') #, skip_blank_lines=True)
#print(df.head(3))
print(len(df))
